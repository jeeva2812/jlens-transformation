...
fill('f_model',uniq('model'));fill('f_matrix',uniq('matrix'));fill('f_layer',uniq('layer'));
fill('f_family',uniq('family'));fill('f_verdict',uniq('verdict'));
...
function render(){
  ...
  for(const r of DATA){
    ...
    let h='<h3>'+esc(r.model)+' '+esc(r.matrix)+' L'+r.layer+' '+esc(r.family)+'['+r.index+']</h3>';
    h+='<span class="badge v-'+esc(r.verdict)+'">'+esc(r.verdict)+'</span>';
    h+='<span class="badge s-'+esc(st)+'">'+esc(st)+'</span>';
    if(r.hypothesis)h+='<div>hypothesis: <b>'+esc(r.hypothesis)+'</b> score '+r.score.toFixed(1)+' vs thr '+r.null_threshold.toFixed(1)+' (null%% beating '+r.null_pct_beating.toFixed(2)+')</div>';
    ...
    if(r.tokens_in_pos)h+='<div class="toks">in+ '+(r.tokens_in_pos||[]).slice(0,8).map(esc).join(' | ')+'</div>';
