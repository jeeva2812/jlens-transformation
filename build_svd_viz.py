...
D = open("/tmp/svd_data.json").read()

HEAD = r'''<title>Inside the Transport</title>
...
<div class="wrap">
<header>
  <p class="eyebrow">Olmo 3 7B &middot; main &middot; SVD of the Jacobian</p>
  <h1>Inside the Transport</h1>
  <p class="sf">J is decomposed as <code>J = U S V&#7488;</code>. Each index i is an input&rarr;output pair: perturb layer l along <code>V[:,i]</code> and layer 30 moves along <code>U[:,i]</code>, scaled by <code>S[i]</code>. Below, each <code>U[:,i]</code> is read through the model's unembedding, so the tokens are what that direction promotes when it arrives. Both signs are shown because SVD fixes only the joint sign &mdash; the pair of poles is the axis.</p>
</header>
...
<div class="ctl">
  ...
  <input type="range" id="n" min="4" max="64" value="16" step="4" style="width:190px">
  <span id="energy"></span>
</div>
<div class="grid" id="cards"></div>

<h2>Do layers read the same subspace?</h2>
<p class="note">Mean cosine of the principal angles between two layers' top-64 <em>input</em> directions (the <code>V</code> side &mdash; what a layer responds to, not what it emits). 1.0 is identical, and the measured null for two random 64-dimensional subspaces in 4096 dimensions is shown below the table.</p>
<div class="scroll"><table id="ov"></table></div>
<p class="note" id="ovnote"></p>

<footer>
Computed from saved Jacobians with no model in memory &mdash; only the unembedding and final norm are needed once J exists.<br>
...
function render(){
  ...
  document.getElementById('energy').textContent =
    `top-1 holds ${(e.top1*100).toFixed(1)}% of J's energy \u00b7 top-64 holds ${(e.topK*100).toFixed(1)}% \u00b7 effective rank ${e.eff_rank.toFixed(0)} of 4096`;
...
pathlib.Path("out/svd_viz.html").write_text(out)
print("wrote out/svd_viz.html", len(out), "bytes")
